...
class AvrBurner(QWidget):
    def __init__(self):
        ...
        # Fuse read button
        self.read_fuse_btn = QPushButton("Fuse Bits Read")
        self.read_fuse_btn.clicked.connect(self.read_fuses)
        layout.addWidget(self.read_fuse_btn)

        # Fuse set manually
        layout.addWidget(QLabel("Fuse Bits Set Manually:"))
        ...
        # Set fuses button
        self.set_fuse_btn = QPushButton("Set Fuses")
        self.set_fuse_btn.clicked.connect(self.set_fuses)
        layout.addWidget(self.set_fuse_btn)
    ...
    def read_fuses(self):
        ...
        if output:
            lfuse, hfuse, efuse = None, None, None

            for line in output.splitlines():
                if "lfuse" in line.lower() and "0x" in line:
                    lfuse = line.split()[-1].replace("0x", "").upper()
                if "hfuse" in line.lower() and "0x" in line:
                    hfuse = line.split()[-1].replace("0x", "").upper()
                if "efuse" in line.lower() and "0x" in line:
                    efuse = line.split()[-1].replace("0x", "").upper()

            self.lfuse_edit.setText(lfuse if lfuse else "??")
            self.hfuse_edit.setText(hfuse if hfuse else "??")
            self.efuse_edit.setText(efuse if efuse else "??")

            if lfuse and hfuse and efuse:
                self.status_label.setText(f"✅ Fuses OK (E:{efuse}, H:{hfuse}, L:{lfuse})")
                self.status_label.setStyleSheet("color: green;")
            else:
                self.status_label.setText("⚠ Could not read all fuse bits")
                self.status_label.setStyleSheet("color: orange;")

    def set_fuses(self):
        chip = self.chip_combo.currentText()
        port = self.port_combo.currentText()
        lfuse = self.lfuse_edit.text().strip()
        hfuse = self.hfuse_edit.text().strip()
        efuse = self.efuse_edit.text().strip()
        ...
        if lfuse and lfuse != "??":
            avrdude_cmd += ["-U", f"lfuse:w:0x{lfuse}:m"]
        if hfuse and hfuse != "??":
            avrdude_cmd += ["-U", f"hfuse:w:0x{hfuse}:m"]
        if efuse and efuse != "??":
            avrdude_cmd += ["-U", f"efuse:w:0x{efuse}:m"]

        output = self.run_avrdude(avrdude_cmd)
        if "writing" in output.lower() or "safemode" in output.lower():
            self.status_label.setText("✅ Fuses Set Successfully!")
            self.status_label.setStyleSheet("color: green;")
        else:
            self.status_label.setText("❌ Failed to Set Fuses")
